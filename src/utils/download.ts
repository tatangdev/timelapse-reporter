...
const downloadFolder: string = path.join(__dirname, "../../images-4k");
const allFilesFolder: string = path.join(downloadFolder, "all");

// Convert function to TypeScript with type annotations
async function downloadFtpFiles(
  folderPath: string = FTP_INDEX_FOLDER
): Promise<void> {
  try {
    await connectFTP();
    ...
    if (!fs.existsSync(allFilesFolder))
      fs.mkdirSync(allFilesFolder, { recursive: true });
    ...
    for (const file of list) {
      if (file.isFile && isImageFile(file.name)) {
        ...
        if (folder < REPORT_START_DATE) continue; // Skip files before the start date
...
function getFileName(input: string): { folder: string; file: string } {
  ...
  const folder = `${datePart.slice(0, 4)}-${datePart.slice(
    4,
    6
  )}-${datePart.slice(6, 8)}`;
