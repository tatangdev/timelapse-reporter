...
async function checkHealth(
  isSystemCheck: boolean,
  ctx: Context | null
): Promise<void> {
  const currentTime = moment().tz(TIMEZONE);
  const currentHour = currentTime.hour();

  if (isSystemCheck && (currentHour < 9 || currentHour >= 18)) return;
...
async function handleTodayCheck(
  files: FileDetails[],
  ...
  if (todayFiles.length === 0) {
    ...
    ctx.reply(
      `No CCTV images found for today (${currentTime.format(
        "dddd, DD MMMM YYYY"
      )}) ❌`
    );
    ...
    console.log(
      `${todayFiles.length} CCTV images found for today (${todayDate}) ✅`
    );
    ctx.reply(
      `${todayFiles.length} CCTV images found for today (${currentTime.format(
        "dddd, DD MMMM YYYY"
      )}) ✅`
    );
  }
...
async function handleLastHourCheck(
  files: FileDetails[],
  currentTime: moment.Moment
): Promise<void> {
  const oneHourAgo = currentTime.clone().subtract(1, "hour").unix();
  const lastHourFiles = files.filter(
    (file) =>
      file.unixTimestamp >= oneHourAgo &&
      file.unixTimestamp < currentTime.unix()
  );

  const todayDate = currentTime.format("YYYY-MM-DD");
  const savedDate = await redisClient.get(NOTIFY_DATE_KEY);
  const notifyCount = parseInt(
    (await redisClient.get(NOTIFY_COUNT_KEY)) || "0",
    10
  );
  ...
  if (lastHourFiles.length === 0) {
    if (notifyCount < 3) {
      ...
      console.log(
        "No CCTV images found for the last hour, but notification limit reached. ❌"
      );
    }
  } else {
